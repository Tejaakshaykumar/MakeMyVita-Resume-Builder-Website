...
exports.createResume = async (req, res) => {
  const { name, description, templateNo,fontColor,fontFamily,templateColor, personalDetails, educationDetails, projectDetails, experienceDetails, courseDetails, skillDetails,studentDetails, additional } = req.body;
  try {
...
exports.deleteResume = async (req, res) => {
  ...
  try {
    ...
    await Promise.all([
      ...
      StudentDetails.findByIdAndDelete(resume.personalDetails._id),
      AdditonalDetails.deleteMany({ _id: { $in: resume.additionalDetails.map(skill => skill._id) } }),
    ]);
