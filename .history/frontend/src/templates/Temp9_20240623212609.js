...
const Temp9 = ({
  ...
  const templateStyle = {
    color: ResumeDetails?ResumeDetails.fontColor: '',
    fontFamily: ResumeDetails? ResumeDetails.fontFamily: '',
  };
  const templateColor ={
    background: ResumeDetails?ResumeDetails.templateColor : '' 
  }
  return (
      ...
      <section className="education">
        ...
        <ul>
          {courses.map((course, index) => (
            <li key={index}><strong></strong>{course.skillName || 'Course Name'}</li>
          ))}
        </ul>
      </section>
