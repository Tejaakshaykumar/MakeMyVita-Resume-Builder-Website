...
const Step1Template = ({ goToNextStep,handleChange,ResumeDetails }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedTemplate, setSelectedTemplate] = useState(null);
  ...
  const handleTemplateClick = (template) => {
    ResumeDetails.selectedTemplate=selectedTemplate;
  };
    ...
    <div className="step-container1">
      ...
      <div className="form-row">
          <div className="form-group">
            ...
            <input type="text" name="ResumeName" value={ResumeDetails.ResumeName} onChange={handleChange} />
          </div>
          <div className="form-group">
            <label>Description:</label>
            <input type="text" name="Description" value={ResumeDetails.Description} onChange={handleChange} />
          </div>
        </div>
      <div className="template-list">
        {filteredTemplates.map(template => (
          <div key={template.id} className="template-item">
            ...
            <div className="template-buttons">
              ...
              <button onClick={() => { handleTemplateClick(template); goToNextStep(); }}>Select</button>
            </div>
          </div>
        ))}
      </div>
