...
function ResumeView() {
  ...
  const handleDownload =async (format)=>{
    ...
    if (resumeElement) {
      ...
      if (format === "pdf") {
        const width = 2480; // A4 width in pixels
      const height = 3508; // A4 height in pixels

      const pdf = new jsPDF({
        unit: 'px', // Use pixels for dimensions
        format: [width, height] // A4 dimensions in pixels
      });
  ...
  return (
    <div className="resume" id="resume-content" style={{paddingTop:"1in"}}>
